...
Fream은 확장 가능한 웹 서비스를 위한 견고한 인프라를 제공하는 Spring Boot 기반의 백엔드 애플리케이션입니다. 이 프로젝트는 [이전 버전](https://github.com/junhyeong9812/Improve_Fream_Back)을 기반으로 아키텍처와 코드 구조를 전면 개선한 버전입니다.

## 프로젝트 재구성 배경

### 기존 한계점
- 단순한 e-Commerce 구조로 실제 KREAM과는 다른 로직/연관관계
- 모든 기능이 하나의 서비스에 집중되어 유지보수가 어려움
- 실무 수준의 배포(클라우드, Docker) 아키텍처 미흡
...
3. **개발 프로세스 개선**
   - Git 브랜치 전략 도입
   - 코드 리뷰 프로세스 체계화
   - 테스트 자동화 이 프로젝트는 최신 기술을 활용하고 클라우드 네이티브 애플리케이션을 위한 모범 사례를 따릅니다.
