...
/**
 * 상품 상세 이미지 명령(Command) 서비스
 * 상품 상세 이미지의 생성, 삭제 기능을 제공합니다.
 */
@Service
@Transactional
...
public class ProductDetailImageCommandService {
     ...
     * @return 생성된 상품 상세 이미지 엔티티
     * @throws ProductException 상품 상세 이미지 생성 실패 시
     */
    public ProductDetailImage createProductDetailImage(String imageUrl, ProductColor productColor) {
        log.info("상품 상세 이미지 생성 요청 - 색상ID: {}, 이미지URL: {}",
        ...
        try {
            ...
            ProductDetailImage savedImage = productDetailImageRepository.save(detailImage);
            log.info("상품 상세 이미지 생성 성공 - 이미지ID: {}", savedImage.getId());
            return savedImage;
        } catch (Exception e) {
            log.error("상품 상세 이미지 생성 중 예상치 못한 오류 발생 - 색상ID: {}, 이미지URL: {}",
                    productColor.getId(), imageUrl, e);
            throw new ProductException(ProductErrorCode.IMAGE_UPLOAD_FAILED,
                    "상품 상세 이미지 생성 중 오류가 발생했습니다.", e);
        }
    }
